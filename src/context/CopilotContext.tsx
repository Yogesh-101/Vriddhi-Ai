...
export const CopilotProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const triggerInvoiceDelivery = async (invoiceId: string, channels: ('Email' | 'WhatsApp' | 'Telegram')[]) => {
    ...
    try {
      await sendNotification({
        type: 'delivery',
        message: `Invoice ${invoiceId} dispatched securely to ${client?.name || 'client'}.`,
        channels,
      ...
      console.warn('Invoice delivery notification failed:', e);
    }
  };
  ...
  const triggerPaymentReminder = async (invoiceId: string, channels: ('Email' | 'WhatsApp' | 'Telegram')[]) => {
    ...
    try {
      await sendNotification({
        type: 'reminder',
        message: `Payment overdue reminder for Invoice ${invoiceId}. Please settle at earliest.`,
        channels,
        recipientEmail: emailTo,
        recipientPhone: phoneTo,
        invoiceId,
      ...
      console.warn('Payment reminder notification failed:', e);
    }
  };
  ...
  const triggerEscalatedReminder = async (invoiceId: string, level: 1 | 2 | 3, channels: ('Email' | 'WhatsApp' | 'Telegram')[]) => {
    ...
    const levelDescriptions = {
      1: `Friendly payment prompt for outstanding invoice ${invoiceId}.`,
      2: `URGENT: Invoice ${invoiceId} overdue. 1.5% daily late penalty applies.`,
      3: `STATUTORY NOTICE: Legal escalation initiated for invoice ${invoiceId}.`,
    };
    ...
    try {
      ...
      console.warn('Escalated reminder notification failed:', e);
    }
  };
