...
export function Invoices() {
  ...
  useEffect(() => {
    if (invoices.length === 0 || clientsVendors.length === 0 || triggerLogs.length === 0) return;
    const now = new Date();
    invoices.forEach(inv => {
      if (inv.status === 'Overdue' || (inv.status === 'Sent' && inv.dueDate && new Date(inv.dueDate) < now)) {
        const alreadySent = triggerLogs.some(
          log => log.invoiceId === inv.id && log.type === 'reminder' &&
          new Date(log.timestamp).toDateString() === now.toDateString()
        );
        if (!alreadySent) {
          const client = clientsVendors.find(c => c.id === inv.clientId);
          persistNotification({
            id: `ntf-auto-${Date.now()}-${inv.id}`,
            timestamp: now.toISOString(),
            type: 'reminder',
            invoiceId: inv.id,
            clientName: client?.name || 'Client',
            message: `Auto-triggered: Payment overdue for Invoice ${inv.id} (Due: ${inv.dueDate})`,
  ...
  }, [invoices, clientsVendors]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const addInvoice = async (invoice: any) => {
    ...
    if (data && data[0]) {
      ...
      setInvoices(prev => [{ id: `inv-${Math.random().toString(36).substr(2, 9)}`, ...invoice }, ...prev]);
    }
  };
  ...
  const persistNotification = async (log: any, dispatch = false) => {
    setTriggerLogs(prev => [log, ...prev]);
    await supabase.from('notification_logs').insert(log);

    if (dispatch && log.destination) {
      const channels = String(log.destination).split(',').map((s: string) => s.trim()) as ("Email" | "Telegram" | "WhatsApp")[];
      const client = clientsVendors.find(c => c.name === log.clientName);
      try {
        ...
        console.warn("Live notification dispatch:", e);
      }
    }
  };

  const triggerInvoiceDelivery = (id: string, methods: string[]) => {
    const client = clientsVendors.find(c => c.id === invoices.find(i => i.id === id)?.clientId);
    persistNotification({
      id: `ntf-${Date.now()}`,
      ...
      destination: methods.join(', '),
      status: 'delivered',
      simulated: false
    }, true);
  };

  const triggerPaymentReminder = (id: string, methods: string[]) => {
  ...
  const triggerEscalatedReminder = (id: string, method: string) => {
    const inv = invoices.find(i => i.id === id);
    const client = clientsVendors.find(c => c.id === inv?.clientId);
    persistNotification({
      id: `ntf-${Date.now()}`,
      ...
      invoiceId: id,
      clientName: client?.name || 'Client',
      message: `Escalated reminder dispatched - Invoice ${id}`,
      destination: method,
      status: 'delivered',
      simulated: false
    }, true);
  ...
  const handleCreateAndSave = (status: 'Draft' | 'Sent' | 'Paid') => {
    ...
    if (status === 'Sent') {
      ...
      persistNotification({
        ...
        invoiceId: 'NEW',
        clientName: client?.name || 'Client',
        message: `New invoice generated and delivered to ${client?.email || 'client'}`,
        destination: 'Email, Telegram',
        status: 'delivered',
        simulated: false
      }, true);
                      ...
                      <TableCell>
                        <div className="flex items-center justify-center gap-2">
                           ...
                           <Button 
                             ...
                             onClick={() => {
                               triggerInvoiceDelivery(inv.id, ['Email', 'WhatsApp']);
                               showToast("Success: Invoice PDF pushed securely via Twilio and SendGrid SMTP triggers.");
                             }}
                           >
                           ...
                           {inv.status === 'Overdue' && (
                             ...
                             <Button 
                               ...
                               onClick={() => {
                                 triggerPaymentReminder(inv.id, ['Email', 'WhatsApp']);
                                 showToast("Gentle payment reminder dispatched successfully via Twilio/SendGrid channels.");
                               }}
                             >
                              ...
                              <Button 
                                ...
                                onClick={() => {
                                  triggerEscalatedReminder(inv.id, 'Email');
                                  showToast("MANDATORY ESCALATING REMINDER Fired: High-priority dunning alert dispatched.");
                                }}
                              >
